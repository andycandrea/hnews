...
describe Article do
  ...
  describe '#num_comments' do
    subject { create(:article, :has_url) }

    it 'displays the number of comments on an article' do
      subject.num_comments.should == 0

      comment = create(:comment, commentable: subject)
      subject.num_comments.should == 1

      create(:comment, commentable: comment)
      subject.reload.num_comments.should == 2
    end
  end
end
